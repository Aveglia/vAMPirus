...
<br>
<br>
<div class="rectangle"><h2 style="color:white">&nbsp;&nbsp;Read Processing Info/Results</h2></div>
<br>
...
if (params$skipReadProcessing == "true" || params$skipMerging == "true" ) {
    writeLines("\n--------------------------------------------------------------\n")
    message("Read processing skipped.")
    writeLines("\n--------------------------------------------------------------\n")
} else {
    if (params$skipAdapterRemoval == "false") {
        ...
        writeLines("\n--------------------------------------------------------------\n")
        message("Adapter removal skipped.")
        writeLines("\n--------------------------------------------------------------\n")
    }
}
```
...
if [ `ls *_ASV_Grouping.csv | wc -l` -ge 1 ];then
  cat *_ASV_Grouping.csv >asv_medfile-data.csv
  ...
  cat *_AminoType_Grouping.csv >amino_medfile-data.csv
...
if [ `ls -1 *summary_for_plot.csv | wc -l` -ge 1 ];then
...
write.csv(shannondata5_2, file=paste(params$projtag, "shannon-data.csv", sep="_"), row.names = F)
...
write.csv(simpsondata5_2, file=paste(params$projtag, "simpson-data.csv", sep="_"), row.names = F)
...
mind5<-min(data5$sum)
index <- rarefy(data5[,2:cols], sample=0.95*mind5)
...
write.csv(richdata5_2, file=paste(params$projtag, "richness-data.csv", sep="_"), row.names = F)
...
bray.distance <- vegdist(intermediate, method="bray")
...
write.csv(df2, file=paste(params$projtag, "distance-to-centroid-data.csv", sep="_"), row.names = F)

if (params$stats == "true" ) {
    adn <- adonis2(bray.distance~data5$treatment)
    adn
} else {
    print("Stats skipped. To toggle on use \"--stats run\" in vAMPirus launch command")
}
...
<div class="rectangle"><h2 style="color:white">&nbsp;&nbsp;NMDS or PCoA Plots</h2></div>
...
if (nrow(datax) > 2) {
    ...
    write.csv(data.scores.2, file=paste(params$projtag, "MDS2d-data.csv", sep="_"), row.names = F)
    ...
    write.csv(comp2, file=paste(params$projtag, "PCoA2d-data.csv", sep="_"), row.names = F)
    }
}
```
...
if (nrow(datax) > 2) {
    ...
    data.scores3 <- as.data.frame(scores(MDS3)$sites)
    ...
    write.csv(data.scores.3, file=paste(params$projtag, "MDS3d-data.csv", sep="_"), row.names = F)
    ...
    write.csv(comp2, file=paste(params$projtag, "PCoA3d-data.csv", sep="_"), row.names = F)
    }
}
```
...
write.csv(datalong, file=paste(params$projtag, "relative-abundance-data.csv", sep="_"), row.names = F)
```
...
if (params$skipPhylogeny == "true") {
    ...
    p2 <- p1 + geom_point(data = metat2, aes(x = x, y = y, label = id, color = GroupID)) + geom_treescale(y=-50, offset=10)
    ...
    p2 <- p1 + geom_point(data = taxDF, aes(x = x, y = y, label = label, color = tax)) + geom_treescale(y=-50, offset=10)
    ...
    gg
} else if ((params$asvTClust == "true" || params$aminoTClust == "true") && (params$type == "ASV" || params$type == "AminoType") && (params$nodeCol == "TC")) {
    taxMap=read.csv("phygroup.csv", header=T, check.names = F)
    tree=read.tree("tree.txt")
    p1 <- ggtree(tree)
    id <- tree$tip.label
    dat <- tibble::tibble(id = id)
    ...
    p2 <- p1 + geom_point(data = taxDF, aes(x = x, y = y, label = label, color = tax)) + geom_treescale(y=-50, offset=10)
    gg = ggplotly(p2, tooltip = c("label","tax"))
    gg <- gg %>% config(toImageButtonOptions=list(format='svg',filename='Tree_Phygroup_colors', height= 500, width= 800, scale= 1))
    gg
} else {
    ...
    dat <- tibble::tibble(id = id)
    metat <- p1$data %>% dplyr::inner_join(dat, c('label' = 'id'))
    p2 <- p1 + geom_point(data = metat, aes(x = x, y = y, label = id, color = id)) + geom_treescale(y=-50, offset=10)
...
```
This tree is a maximum likelihood tree made with IQTREE2 and the parameters you specified in the vampirus.config file. Also, this is an interactive tree, you can zoom in and hover on nodes to know the sequence ID. For a better visualization of this tree, you can find the *.treefile with bootstrap support values within the results directory and visualize using programs like FigTree or ITOL. Check out the use of the option --nodeCol to color nodes by taxonomy hit, MED group or phyloGroup.

<br>
<br>
<br>
<div class="rectangle"><h2 style="color:white">&nbsp;&nbsp;Phylogeny-based Clustering Results</h2></div>
# Phylogeny-based Clustering Summary Table
```{bash bash_phy_table, include=FALSE}
if [[ $(ls | grep -c "_phylogroup.csv") -eq 1 ]];then
  name=$( ls *_counts.csv | awk -F "_counts.csv" '{print $1}')
  awk -F "," '{print $2}' *_phylogroup.csv | sort | uniq | sort -g >group.list
  for x in $(cat group.list);do
    echo "${x}" >> ${x}.col
    grep -w "$x" *_phylogroup.csv | awk -F "," '{print $1}' | sort >> ${x}.col
  done
  paste -d "," *.col > phylogroup.csv
  rm *.col
  cp phylogroup.csv ./"$name"-phylogroup.csv
else
    echo "Not phylogeny based clustering"
fi
```

```{r phyloGroup_table, echo=FALSE}
if ((params$asvTClust == "true") && (params$type == "ASV")) {
    phylogroup=read.csv("phylogroup.csv", header = T)
    knitr::kable(phylogroup, digits = 2, align = 'c', booktabs = TRUE, caption = "Table: phyloGroup Table") %>%
    kable_styling(font_size = 12, full_width = F)%>%
    scroll_box(width = "100%", height = "100%")
}
if ((params$aminoTClust == "true") && (params$type == "AminoType")) {
    phylogroup=read.csv("phylogroup.csv", header = T)
    knitr::kable(phylogroup, digits = 2, align = 'c', booktabs = TRUE, caption = "Table: phyloGroup Table") %>%
    kable_styling(font_size = 12, full_width = F)%>%
    scroll_box(width = "100%", height = "100%")
...
```{r phylo_plot, echo=FALSE}
if ((params$asvTClust == "true" || params$aminoTClust == "true") && (params$type == "ASV" || params$type == "AminoType")) {
  #medgroup=read.csv("uVP_AminoType_Groupingcounts.csv", check.names=FALSE)
  phylogroup=read.csv("phylogroupcounts.csv", header = T, check.names=FALSE)
  phydata <- phylogroup %>% select(-2)
  phydata2 <- aggregate(.~phyloGroup_ID, phydata, sum)
  phydata3 <-as.data.frame(t(phydata2))
  phydata3$sample <- row.names(phydata3)
  colnames(phydata3)<- as.matrix(phydata3[1,])
  as.data.frame(phydata3)
  phydata3 <- phydata3[-1,]
  phydata3 <- phydata3 %>%
    rename(sample=phyloGroup_ID)
  phydata3dim <- dim(phydata3)
  ##Loading metadata
  samples <- read.csv(sample_metadata, header = TRUE)
  #samples <- read.csv("rna_virus_meta.csv", header = TRUE)
  ##Combining data and metadata
  phydata4 <- merge(phydata3, samples, by="sample")
  #if "phydata4" removed it does not work
  #phydata4
  dim_phydata4 <- dim(phydata4)
  dim_samples <- dim(samples)
  cols <- dim_phydata4[2]-dim_samples[2]+1
  first <-colnames(phydata4)[2]
  last <- colnames(phydata4)[cols]
  phydata4[,2:cols] <- lapply(phydata4[,2:cols], as.character)
  phydata4[,2:cols] <- lapply(phydata4[,2:cols], as.numeric)
  #Calculate total reads per sample
  phydata5 <- phydata4%>%
    mutate(sum=select(.,2:cols)%>%
             apply(1, sum, na.rm=TRUE))
  ...
  phydata5 <- phydata5 %>%
    filter(sum>nfil)
     #can cause errors
  phydata5dim <-dim(phydata5)
  minreads<-min(phydata5$sum)

  dataz <- decostand(phydata5[,2:cols],method="total") #method 'total' normalizes data to sum up to 1
  dataz$sample <- phydata5$sample
  metadata <- phydata5[,(cols+1):phydata5dim[2]]
  metadata$sample <- phydata5$sample
  datay <- merge(dataz, metadata, by="sample")
  datalong <- datay %>%
    tidyr::gather(first:last, key=hit, value=reads)
  ...
  write.csv(datalong, file=paste(params$projtag, "phylogroup-relative-abundance-data.csv", sep="_"), row.names = F)
}
```
...
# phyloGroup-based Rarefaction Curves
...
if ((params$asvTClust == "true" || params$aminoTClust == "true") && (params$type == "ASV" || params$type == "AminoType")) {
##Rarefaction curves
rarefaction <- rarecurve(phydata5[,2:cols])

##rarefied dataset
raredata <- as.data.frame(rrarefy(phydata5[,2:cols], sample=minreads))
}
```
...
<div class="rectangle"><h2 style="color:white">&nbsp;&nbsp;phyloGroup-based Diversity Analyses Plots</h2></div>

<br>
<br>

# phyloGroup-based Shannon diversity
...
if ((params$asvTClust == "true" || params$aminoTClust == "true") && (params$type == "ASV" || params$type == "AminoType")) {
    ...
    index <-diversity(raredata, index= "shannon")
    shannondata5 <- as.data.frame(index)
    shannondata5$sample<- phydata5$sample
    shannondata5_2 <- merge(shannondata5, metadata, by="sample")
    ...
    write.csv(shannondata5_2, file=paste(params$projtag, "phylogroup-shannon-data.csv", sep="_"), row.names = F)
}
```
...
# phyloGroup-based Simpson diversity
...
if ((params$asvTClust == "true" || params$aminoTClust == "true") && (params$type == "ASV" || params$type == "AminoType")) {
    index <- diversity(raredata, index= "simpson")
    simpsondata5 <- as.data.frame(index)
    simpsondata5$sample<- phydata5$sample
    simpsondata5_2 <- merge(simpsondata5, metadata, by="sample")
    ...
    write.csv(simpsondata5_2, file=paste(params$projtag, "phylogroup-simpson-data.csv", sep="_"), row.names = F)
}
```
...
# phyloGroup-based Richness

<br>
```{r phy_diversity_analysis3a, echo=FALSE}
if ((params$asvTClust == "true" || params$aminoTClust == "true") && (params$type == "ASV" || params$type == "AminoType")) {
    mind5<-min(phydata5$sum)
    index <- rarefy(phydata5[,2:cols], sample=0.95*mind5)
    rarerichnessdata5 <- as.data.frame(index)
    rarerichnessdata5$sample <-phydata5$sample
    richdata5_2 <- merge(rarerichnessdata5, metadata, by="sample")
    ...
    write.csv(richdata5_2, file=paste(params$projtag, "phylogroup-richness-data.csv", sep="_"), row.names = F)
}
```

```{r phylo_diversity_analysis3b, echo=FALSE}
if ((params$asvTClust == "true" || params$aminoTClust == "true") && (params$type == "ASV" || params$type == "AminoType")) {
    if (params$stats == "true" ) {
        richaov <- aov(index ~ treatment, data= richdata5_2)
        st <- shapiro.test(resid(richaov))
...
# phyloGroup-based Distance To Centroid
<br>
```{r phylo_distance2a, echo=FALSE}
if ((params$asvTClust == "true" || params$aminoTClust == "true") && (params$type == "ASV" || params$type == "AminoType")) {
    ##Distance
    intermediate <- raredata
    ...
    write.csv(df2, file=paste(params$projtag, "phylogroup-distancetocentroid-data.csv", sep="_"), row.names = F)
}
```
```{r phylo_distance2b, echo=FALSE}
if ((params$asvTClust == "true" || params$aminoTClust == "true") && (params$type == "ASV" || params$type == "AminoType")) {
    if (params$stats == "true" ) {
        adn <- adonis2(bray.distance~phydata5$treatment)
        adn
    } else {
        print("Stats skipped. To toggle on use \"--stats run\" in vAMPirus launch command")
    }
...
<div class="rectangle"><h2 style="color:white">&nbsp;&nbsp;phyloGroup-based NMDS or PCoA Plots</h2></div>

<br>

# phyloGroup-based 2D NMDS or PcOA

<br>
```{r phylo_nmds2d, echo=FALSE}
if ((params$asvTClust == "true" || params$aminoTClust == "true") && (params$type == "ASV" || params$type == "AminoType")) {
##NMDS
datax <- decostand(raredata,method="total") #method 'total' normalizes data to sum up to 1 --data5[,2:cols]
if (nrow(datax) > 2) {
    ...
    data.scores <- as.data.frame(scores(MDS)$sites)
    data.scores$sample <- phydata5$sample
    data.scores.2 <- merge(data.scores, metadata, by="sample")
    ...
    write.csv(data.scores.2, file=paste(params$projtag, "phylogroup-MDS2d-data.csv", sep="_"), row.names = F)
      ...
      write.csv(comp2, file=paste(params$projtag, "phylogroup-PCoA2d-data.csv", sep="_"), row.names = F)
    }
    }
}
```
...
# phyloGroup-based 3D NMDS and PCoA

<br>

```{r phylo_nmds3d, echo=FALSE}
if ((params$asvTClust == "true" || params$aminoTClust == "true") && (params$type == "ASV" || params$type == "AminoType")) {
    if (nrow(datax) > 2) {
    MDS3 <- metaMDS(datax,
                   distance = "bray",autotransform = TRUE,
    ...
    data.scores3 <- as.data.frame(scores(MDS3)$sites)
    data.scores3$sample <- phydata5$sample
    data.scores.3 <- merge(data.scores3, metadata, by="sample")
    p3d <- plot_ly(data.scores.3, x=~NMDS1, y=~NMDS2, z=~NMDS3, text=~paste("Sample: ", sample),
            color=~treatment, colors=mycol,
            mode = 'markers', symbol = ~treatment, symbols = c('square','circle'),
    ...
    write.csv(data.scores.3, file=paste(params$projtag, "phylogroup-MDS3d-data.csv", sep="_"), row.names = F)
      ...
      write.csv(comp2, file=paste(params$projtag, "phylogroup-PCoA3d-data.csv", sep="_"), row.names = F)
    }
    }
}
```
...
<div class="rectangle"><h2 style="color:white">&nbsp;&nbsp;Minimum Energy Decomposition (MED) Results</h2></div>
...
if [[ $(ls | grep -wc "asv_medfile-data.csv") -eq 1 ]];then
  awk -F "," '{print $2}' asv_medfile-data.csv | sort | uniq | sort -g >group.list
  for x in $(cat group.list);do
    echo "${x}" >> ${x}.col
    grep -w "$x" asv_medfile-data.csv | awk -F "," '{print $1}' | sort >> ${x}.col
  done
  paste -d "," *.col > asv_medtable.csv
  #ln -s ./asv_medtable.csv deseq_medtable.csv
  rm *.col
else
    echo "Not MED"
fi

if [[ $(ls | grep -wc "amino_medfile-data.csv") -eq 1 ]];then
  awk -F "," '{print $2}' amino_medfile-data.csv | sort | awk -F "Group" '{print $2}' | sort -n | uniq > group.list
  for x in $(cat group.list);do
    gr="Group"${x}""
    echo "${gr}" > ${x}.col
    grep -w "$gr" amino_medfile-data.csv | awk -F "," '{print $1}' | sort >> ${x}.col
  done
  paste -d "," *.col > amino_medtable.csv
  #ln -s ./amino_medtable.csv deseq_medtable.csv
  rm *.col
else
      echo "Not MED"
fi
...
if (params$asvMED == "true" && params$type == "ASV"){
    #med_group=read.csv("asv_medfile-data.csv", header = F)
...
if (params$aminoMED == "true" && params$type == "AminoType"){
  #med_group=read.csv("amino_medfile-data.csv", header = F)
  #colnames(med_group) <- c("SequenceID", "Group", "Sequence")
  #paged_table(med_group,options = list(rows.print = 20))
...
## MED Group Relative Abundance Plot
...
## MED Group-based Rarefaction Curves</h2></div>
...
## MED Group-based Shannon diversity
...
if ((params$asvMED == "true" || params$aminoMED == "true") && (params$type == "ASV" || params$type == "AminoType")){
    ...
    sh <- sh %>% config(toImageButtonOptions=list(format='svg',filename='ShannonDiv', height= 500, width= 800, scale= 1))
    sh
    write.csv(shannondata5_2, file=paste(params$projtag, "MEDgroup-shannon-data.csv", sep="_"), row.names = F)
}
```
...
## MED Group-based Simpson diversity
...
if ((params$asvMED == "true" || params$aminoMED == "true") && (params$type == "ASV" || params$type == "AminoType")){
    ...
    s <- s %>% config(toImageButtonOptions=list(format='svg',filename='SimpsonDiv', height= 500, width= 800, scale= 1))
    s
    write.csv(simpsondata5_2, file=paste(params$projtag, "MEDgroup-simpson-data.csv", sep="_"), row.names = F)
}
```
...
## MED Group-based Richness

<br>
```{r med_diversity_analysis3a, echo=FALSE}
if ((params$asvMED == "true" || params$aminoMED == "true") && (params$type == "ASV" || params$type == "AminoType")){
    mind5<-min(meddata5$sum)
    index <- rarefy(meddata5[,2:cols], sample=0.95*mind5)
    rarerichnessdata5 <- as.data.frame(index)
    ...
    ri <- ri %>% config(toImageButtonOptions=list(format='svg',filename='SpeciesRich', height= 500, width= 800, scale= 1))
    ri
    write.csv(richdata5_2, file=paste(params$projtag, "MEDgroup-richness-data.csv", sep="_"), row.names = F)
}
```
...
## MED Group-based Distance To Centroid
<br>
...
if ((params$asvMED == "true" || params$aminoMED == "true") && (params$type == "ASV" || params$type == "AminoType")){
    ...
    cen <- cen %>% config(toImageButtonOptions=list(format='svg',filename='Dispersion', height= 500, width= 800, scale= 1))
    cen
    write.csv(df2, file=paste(params$projtag, "MEDgroup-distancetocentroid-data.csv", sep="_"), row.names = F)
}
```
```{r med_distance2b, echo=FALSE}
if ((params$asvMED == "true" || params$aminoMED == "true") && (params$type == "ASV" || params$type == "AminoType")){
    if (params$stats == "true" ) {
        adn <- adonis2(bray.distance~meddata5$treatment)
        adn
    } else {
        print("Stats skipped. To toggle on use \"--stats run\" in vAMPirus launch command")
    }
...
<div class="rectangle"><h2 style="color:white">&nbsp;&nbsp;MED Group-based NMDS or PCoA Plots</h2></div>

<br>

## MED Group-based 2D NMDS or PCoA
...
if (nrow(datax) > 2) {
        ...
        write.csv(data.scores.2, file=paste(params$projtag, "MEDgroup-MDS2d-data.csv", sep="_"), row.names = F)
          ...
          fig <- fig %>% config(toImageButtonOptions=list(format='svg',filename='2D_PostMED_PCoA', height= 500, width= 800, scale= 1))
          fig
          write.csv(comp2, file=paste(params$projtag, "MEDgroup-PCoA2d-data.csv", sep="_"), row.names = F)
        }
    }
}
```
...
## MED Group-based 3D NMDS or PCoA
...
if ((params$asvMED == "true" || params$aminoMED == "true") && (params$type == "ASV" || params$type == "AminoType")){
    ...
    write.csv(data.scores.3, file=paste(params$projtag, "MEDgroup-MDS3d-data.csv", sep="_"), row.names = F)
      ...
      write.csv(comp2, file=paste(params$projtag, "MEDgroup-PCoA3d-data.csv", sep="_"), row.names = F)
    }
    }
}
```
...
<div class="rectangle"><h2 style="color:white">&nbsp;&nbsp;MED Group Representative Phylogenetic Tree</h2></div>
...
if (params$skipPhylogeny == "true") {
    ...
    p2 <- p1 + geom_point(data = metat, aes(x = x, y = y, label = id, color=label)) + geom_treescale(y=-50, offset=10)
    ...
    p2 <- p1 + geom_point(data = metat2, aes(x = x, y = y, label = ID, color = label)) + geom_treescale(y=-50, offset=10)
    ggplotly(p2, tooltip = "label")
