---
title: "vAMPirus Report `!r commandArgs(trailingOnly=T)[1]`"
...
params:
  ...
  heatmap: !r commandArgs(trailingOnly=T)[5]  # heatmap
  tax: !r commandArgs(trailingOnly=T)[6]      # tax
  tree: !r commandArgs(trailingOnly=T)[7]     # tree
---
<style>
.rectangle {
  height: 37px;
...
# Filter by treatment
h <- data5 %>%
  filter(treatment == "H") ##variable needed to be defined by config
writeLines(c("Mean"))
mean(h$sum)
writeLines(c("Min"))
min(h$sum)
...
```{r rarefaction, echo=FALSE, cache=TRUE}
##Rarefaction curves
...
simmatrix2 <- simmatrix %>%
  gather(1:159, key=sequence, value=similarity)
...
tree=read.newick(params$tree)
ggtree(tree)+
  geom_nodepoint(color="black", size=1.4, alpha=.7)+
  geom_treescale()+
  geom_rootedge(rootedge = .1)+
  geom_nodelab(hjust=.005)+
  geom_tiplab(align=TRUE, geom="label", offset=.3, hjust=.5, color="black")
