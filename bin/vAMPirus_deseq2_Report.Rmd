...
resraw <- results(ddsraw, C)
resOrdered <- resraw[order(resraw$pvalue),]


resnames <- as.data.frame(resultsNames(ddsraw))
x <- nrow(resnames)
for (v in 2:x) {
  z <- resnames[v, ]
  res1 <- results(ddsraw, name = "treatment_Acropora_vs_Coralivore", cooksCutoff = TRUE)
  resLFC <- lfcShrink(ddsraw, coef=z, type="apeglm")
  write.csv(as.data.frame(resLFC), paste0(params$projtag,"deseq2_LFCshrunk_apeglm_results", z,".csv"))
  plotMA(resraw)
}
