...
bartlett.test(index ~ treatment, data= shannondata5_2)
```
<br>
<br>
<br>
...
summary(simpsonaov)
plot(simpsonaov)
hist(resid(simpsonaov))
shapiro.test(resid(simpsonaov))
bartlett.test(index ~ treatment, data= simpsondata5_2)
```
<br>
<br>
<br>
...
richaov <- aov(index ~ treatment, data= rarerichnessdata5_2)
summary(richaov)
plot(richaov)
hist(resid(richaov))
shapiro.test(resid(richaov))
bartlett.test(index ~ treatment, data= rarerichnessdata5_2)
```
<br>
<br>
<br>
...
bray <- vegdist(intermediate, method="bray",autotransform=TRUE)

adn <- adonis(dist.mat~data5$treatment)
adn
