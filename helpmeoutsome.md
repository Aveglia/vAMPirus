...
The major objective of vAMPirus is to make it easy for a user to analyze their data is many different ways. One major and sometimes difficult step in analyzing virus amplicon sequence data is the method to use to identifying or define different viral "species". vAMPirus has three different ways to treat your virus amplicon data  
    ...
    2. Nucleotide-based clustering into OTUS (nOTUs) -
        ...
        all subsequent processes (e.g. counts, phylogeny, taxonomy, report generation, etc.)
            ...
            // Percent similarity to cluster nucleotide ASV sequences
                clusterNuclID=".85"
            // List of percent similarities to cluster nucleotide ASV sequnces - must be seperated by a comma (ex. ".95,.96")
                clusterNuclIDlist=""
        ...
        Using "--clusterNuclIDlist" will override the single percentage clustering and vAMPirus will generate seperate nOTUs fastas based on 85%, 90% and 96% nucleotide similarity, you could theoretically cluster by any amount
